...
function N2(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType) #choose two consecutive free truck nodes and puts them randomly in truck tour    #OK
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
    free_tnodes = setdiff(tnodes_loc, chrm.LLnodes)
...
function N3(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)   #swap two truck nodes 
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
...
function N5(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)   #swap two consecutive truck nodes with another truck node 
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
    free_tnodes = setdiff(tnodes_loc, chrm.LLnodes)
    indices = Int[]
...
function N6(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)        #Truck swap 2-2
    c = copy(chrm.genes)
    indices = Int[]
    i = 1
...
function N7(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)      #Truck swap 2-2
    c = copy(chrm.genes)
    indices = Int[]
    i = 1
    @inbounds while i < n_nodes
...
function N8(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)       #Truck swap 2-2
    c = copy(chrm.genes)
    indices = Int[]
    i = 1
    @inbounds while i < n_nodes
...
function N9(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)        #Truck swap 2-2
    c = copy(chrm.genes)
    indices = Int[]
    i = 1
    @inbounds while i < n_nodes
...
function N10(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #swap a drone node with either one of randezvous nodes or any truck node between
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
...
function N11(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #in drone route <i,j,k> swap i and j
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
    dnodes_loc = findall(x -> x < 0, c)
...
function N12(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #in drone route <i,j,k> swap k and j
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
    dnodes_loc = findall(x -> x < 0, c)
    if length(dnodes_loc) == 0
...
function N13(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #in drone route <i,j,k> swap k and i
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
    dnodes_loc = findall(x -> x < 0, c)
    if length(dnodes_loc) == 0
...
function N14(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #remove one free truck node and make it a drone node between two consecutive truck nodes
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
    free_tnodes = setdiff(tnodes_loc, chrm.LLnodes)
    indices = Int[]
...
function N14p(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #remove one middle truck node and make it a drone node
    c = copy(chrm.genes)
    middle_tnodes = Int[]
    one_positive = false
    two_positive = false
...
function N15(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #remove a drone node and insert it randomly as a truck node between two consequtive truck nodes
    c = copy(chrm.genes)
    dnodes_loc = findall(x -> x < 0, c)
    if length(dnodes_loc) == 0
        return chrm
...
function N19(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #remove a drone node and insert it randomly as a drone node between two consequtive truck nodes
    c = copy(chrm.genes)
    dnodes_loc = findall(x -> x < 0, c)
    if length(dnodes_loc) == 0
        return chrm
...
function N16(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #swap two drone nodes
    c = copy(chrm.genes)
    dnodes_loc = findall(x -> x < 0, c)
...
function N17(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)   #Swap a truck node with a drone node
    c = copy(chrm.genes)
    dnodes_loc = findall(x -> x < 0, c)
    if length(dnodes_loc) == 0
        return chrm
...
function N18(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #swap two truck arcs
    c = copy(chrm.genes)
    tnodes_loc = findall(x -> x > 0, c)
    idx1 = tnodes_loc[rand(1:length(tnodes_loc))]
    indices = Int[]
...
function N20(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #swap two drone nodes and make them truck nodes
    c = copy(chrm.genes)
    dnodes_loc = findall(x -> x < 0, c)
    if length(dnodes_loc) > 1
        dnode1 = dnodes_loc[rand(1:length(dnodes_loc))]
...
function N21(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  #
    c = copy(chrm.genes)
    dnodes_loc = findall(x -> x < 0, c)
    if length(dnodes_loc) == 0 || length(chrm.LLnodes) == 0
        return chrm
...
function N22(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64}, dEligible::Vector{Int64}, ClosenessT::Matrix{Int64}, ClosenessD::Matrix{Int64}, flying_range::Float64, sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)  ##remove a drone node d, find a random drone pair <i,j,k> change the j to truck and put d in <i,j,d,k> or <i,d,j,k>
    c = copy(chrm.genes)
    dnodes_loc = findall(x -> x < 0, c)
    if length(dnodes_loc) == 0 || length(chrm.LLnodes) == 0
        return chrm
...
function Improve_chromosome(chrm::Chromosome, n_nodes::Int64, TT::Matrix{Float64}, DD::Matrix{Float64},
    ...
    sR::Float64, sL::Float64, penaltyR::Float64, penaltyM::Float64, problem_type::ProblemType)
